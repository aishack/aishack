...
function aishack_custom_nav()
{
    ...
    <li><form id="searchform" method="get" action="/">
		<div>
			<input id="s" name="s" type="text" value="Type and hit enter to search" onfocus="if (this.value == 'Type and hit enter to search') {this.value = '';}" onblur="if (this.value == '') {this.value = 'Type and hit enter to search';}" size="32" tabindex="1">
...
function aishack_header_scripts()
{
	...
	?>
    <script language="javascript" src="/wp-content/themes/aishack/scripts/domtabs.js"></script>
    <?php
}
...
{
	?>
    <script language="javascript" src="/wp-content/themes/aishack/custom.js"></script>
...
function childtheme_override_index_loop()
{
    ...
    <ul>
    	...
    	while(have_posts())
		{
            ...
            <a href="<?php echo get_permalink(); ?>"><img class='post-image-featured' src="/wp-content/themes/aishack/scripts/timthumb.php?src=<?php echo $metavalue[0]; ?>&w=640&h=175" /></a>
		...
		<div class="column <?php if ($counter == 1) { echo 'one'; } elseif ($counter==2) { echo 'two'; } else { echo 'three'; } ?>">
                <div class="clear-fix">
                    ...
                    <?php $metavalues = get_post_meta($post->ID, "thesis_post_image");
						if($metavalues[0]) {?>
							<a href="<?php echo get_permalink(); ?>"><img class='post-image-small' src='/wp-content/themes/aishack/scripts/timthumb.php?src=<?php echo urlencode($metavalues[0]);?>&w=200&h=55&q=75' /></a><?php } ?>
...
function childtheme_override_postheader() {
	global $post, $strReadMore, $enable_below_post_fb_like;
		...
		if($metavalues[0])
		{
			...
			if(!is_singular())
				echo '-multiple';
			echo "' src='/wp-content/themes/aishack/scripts/timthumb.php?src=".urlencode($metavalues[0]);
			if(!is_singular())
				echo "&w=240&h=175&q=75' width='240' height='175' />";
			else
				echo "&w=640&h=175&q=75' width='640' height='175' />";
		}
		?>
	...
	if(is_single())
	{
		...
		if(count($downloads)>0)
		{
			foreach($downloads as $key => $current)
			{
				...
				echo "<li><a href='".$temp[1]."'><img src='/wp-content/themes/aishack/images/";
				if($temp[0]=="zip")
...
function display_pop_posts()
{
	...
	if(have_posts())
	{
		while(have_posts())
		{
			...
			?>
            <a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><img src="/wp-content/themes/aishack/scripts/timthumb.php?src=<?php echo urlencode($metavalue[0]); ?>&w=260&h=75&q=75" class='sidebar-thumb' width='260' height='75' alt="<?php the_title(); ?>" /></a>
            <?php
		}
	}
}
